...
const detectBookAgent = new Agent({
  name: "detect-book",
  instructions: loadPrompt("agents/detect-book/prompt.xml"),
  model: createOpenRouter({
    apiKey: process.env.OPENROUTER_API_KEY
  })("openai/gpt-4.1")
});

const jinaReader = createTool({
  id: "jina-reader",
  ...
  execute: async ({ context }) => {
    const { url } = context;
    try {
      const jinaUrl = url.startsWith("https://r.jina.ai/") ? url : `https://r.jina.ai/${encodeURIComponent(url)}`;
...
const searchBooksMCP = new MCPConfiguration({
  id: "search-books",
  servers: {
    firecrawl: {
      command: "npx",
...
const searchBooksAgent = new Agent({
  name: "search-books",
...
const bookDetectionStep = new Step({
  id: "bookDetection",
  ...
  execute: async ({ context }) => {
    try {
      ...
      const bookDetectAgent = mastra.getAgent("bookDetect");
      if (!bookDetectAgent) {
      ...
      const isBookDetected = bookDetection.isBookDetected && bookDetection.confidence >= confidenceThreshold;
      return {
        isBookDetected,
        confidence: bookDetection.confidence,
        // In a real implementation, you would crop the image here if a book was detected
        croppedImage: isBookDetected ? image : void 0
      };
    } catch (error) {
...
const imageWorkflow = new Workflow({
  ...
  triggerSchema: z.object({
    ...
    options: z.object({
      confidenceThreshold: z.number().min(0).max(1).default(0.7).optional().describe("Minimum confidence threshold for book detection (0-1)"),
      returnRawData: z.boolean().default(false).optional().describe("Whether to include raw data in the response")
    }).optional()
  })
});
imageWorkflow.step(bookDetectionStep).if(async ({ context }) => {
  const bookDetectionStepResult = context.getStepResult("bookDetection");
...
const getBookStep = new Step({
  id: `getBook`,
  inputSchema: searchResultSchema$1,
...
const getBooksStep = new Step({
  id: "getBooks",
  inputSchema: z.object({
    searchResults: z.array(searchResultSchema$1)
  }),
  outputSchema: z.object({
...
const searchBooksStep = new Step({
  id: "searchBooks",
...
textWorkflow.step(searchBooksStep).then(getBooksStep).commit();
